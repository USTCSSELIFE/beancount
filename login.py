...
class Login:
    ...
    def passport(self):
        data = self.session.get(
            "https://passport.ustc.edu.cn/login?service=" + self.service, headers=self.headers)
        data = data.text
        CAS_LT = etree.HTML(data).xpath("//input[@name='CAS_LT']/@value")[0]
        self.session.get(
    ...
    def login(self):
        ...
        try:
            ...
            rsp = self.session.get(
                ticket, headers=self.headers).content.decode("utf-8")
        except:
            print("Login Failed!")
        ...
        self.finepassword = hashlib.md5(
            self.fineReportPw.encode(encoding="utf-8")).hexdigest()
        params = {
            "fine_username": self.stuid,
            "fine_password": self.finepassword,
            "validity": "-1",
        }
        self.session.get("https://jw.ustc.edu.cn/webroot/decision/login/cross/domain",
